...
export const QRCodeModal: React.FC<Props> = ({
  ...
  const { state } = wcs;
  const { showQRCodeModal, QRCode, QRCodeUrl, isMobile, customDesktopWallet } =
    state;
  ...
  useEffect(() => {
    ...
    if (!urlExists && !urlsLoading && isMobile && QRCodeUrl) {
      const fetchFirebase = () => {
        // Set is loading
        setUrlsLoading(true);
  ...
  const renderQRView = () => (
    <>
      ...
      {copied ? (
        ...
        <Text link onClick={copyToClipboard}>
          Copy to clipboard
        </Text>
      )}
      <AppStoreIcons>
  ...
  const handleExtensionAppOpen = (event: React.MouseEvent) => {
    ...
    if (shiftKeyPressed) {
      if (showProvExtId) {
        setProvExtId(PLUGIN_PROVENANCE_WALLET);
  ...
  const renderDesktopView = () => (
    <>
      ...
      {customDesktopWallet && (
        ...
        <WalletRowNonLink
          onClick={(e) => customDesktopWallet.onClick(e, encodedQRCodeUrl)}
        >
          <WalletTitle>{customDesktopWallet.walletTitle}</WalletTitle>
