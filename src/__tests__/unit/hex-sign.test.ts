...
import { isHex, verifySignature } from '../../helpers/chain';
...
describe(`test signing`, () => {
  ...
  it(`should verify unprintable characters signature`, async () => {
    ...
    const signature =
      'a74c498f2de8f2820f975570248f33d2b72521b6b03f0fd721419f0c020f8b9a481ba48c55d38ff70beec306d21506c5a165401d5cc36d5aec3f5c281ec1bdf0';
    const signatureBytes = Buffer.from(signature, 'hex');

    const ecdsaVerifyResult = ecdsaVerify(signatureBytes, hexMessageHash, pubkBytes);
    const wcVerifyResult = await verifySignature(
      hexMessage,
      signatureBytes,
      pubkB64
    );
    ...
    it(`should verify jwt signature`, async () => {
      const jwt =
        'eyJhbGciOiJFUzI1NksiLCJ0eXAiOiJKV1QifQ.eyJzdWIiOiJBaHdjTTdDclRIMGlCYzJwbmhDbHRVSndwb1J1ZnVsUFV1Nnl2ODh3NlFoMSIsImlzcyI6InByb3ZlbmFuY2UuaW8iLCJpYXQiOjE2Nzc1Mjc2NTksImV4cCI6MTY3NzUyODI1OSwiYWRkciI6InRwMTNka2g2bGhlNXp6ZmU0cXdrbDRmcXlmbWg1OHAzNXc1cWF4d3h5In0';
      ...
      const signature =
        '8a2ed625c1c290317b98c653cc166249056010662fedac850c904e1ecf3b718b4c3dbd4e1fe986b62d7ccb31a6e152140442fa02d2a7ee40e6218ca31ee4d307';
      const signatureBytes = Buffer.from(signature, 'hex');

      const ecdsaVerifyResult = ecdsaVerify(
        signatureBytes,
        hexMessageHash,
        pubkBytes
      );
