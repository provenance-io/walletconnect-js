// Values initially setup in helpers.js
const Element = window.demo.Element;
const ListElement = window.demo.ListElement;
...
// NOTE: Next version of wcjs (1.2.8) will export WALLET_LIST
const WALLET_LIST = [
  {
    dev: true,
...
const walletConnectedUpdates = (data) => {
  ...
  const { address, publicKey, connectionEat, connector } =
  ...
  const list = {
    address,
    publicKey,
    'Connection Expires': new Date(connectionEat * 1000),
    bridge: connector.bridge,
    'Wallet App': connector.peerMeta.name,
    'Active Wallet Name': connector.accounts[0].walletInfo.name,
  };
